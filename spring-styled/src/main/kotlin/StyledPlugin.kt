...
class StyledPlugin {
    ...
    companion object {
        ...
        fun onRedefine(ctClass: CtClass, classLoader: ClassLoader) {
            if (!isStyled(ctClass)) return
            println("[StyledPlugin] REDEFINE transform: ${ctClass.name}")
            transform(ctClass)
                ...
                object : Command {
                    override fun executeCommand() {
                        try {
                            ...
                            println("[StyledPlugin] REFRESHED: $className")
                        } catch (e: Exception) {
                            println("[StyledPlugin] REFRESH ERROR: ${e.message}")
                            e.printStackTrace()
                        }
                    }
                },
        ...
        fun isStyled(ctClass: CtClass): Boolean {
            return ctClass.superclassName == "io.exko.styled.Styled"
        }
        ...
        fun transform(ctClass: CtClass) {
            try {
                ...
                println("[StyledPlugin] TRANSFORMING: ${ctClass.name}")
                ...
                println("[StyledPlugin] SUCCESS: ${ctClass.name}")
            } catch (e: Exception) {
                println("[StyledPlugin] ERROR: ${e.message}")
                e.printStackTrace()
            }
        }
    }
