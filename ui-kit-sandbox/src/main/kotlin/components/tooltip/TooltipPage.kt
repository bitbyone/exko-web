...
class TooltipPageController {
    ...
    @GetMapping
    fun render() = view(TooltipPage(), ::UiKitLayout)
}
...
fun Component.Basic() {
    h2 { +"Basic" }
    div {
        Button { id = "btn-basic-tip"; +"Hover me" }
    ...
    div {
        Placement.entries.forEach { placement ->
            div {
                ...
                Button { this.id = id; +"Placement: ${placement.realValue}" }
...
fun Component.DistanceAndSkidding() {
    ...
    div {
        Button { id = "btn-distance-tip"; +"Distance 16" }
    ...
    br
    div {
        Button { id = "btn-skidding-tip"; +"Skidding 24" }
    ...
    br
    div {
        Button { id = "btn-both-tip"; +"Distance 8, Skidding 12" }
...
fun Component.Triggers() {
    ...
    // hover focus (default)
    div {
        Button { id = "btn-default-triggers"; +"Hover/Focus (default)" }
        Tooltip(forId = "btn-default-triggers") { p { +"Default: hover focus" } }
    }
    br
    // click only
    div {
        Button { id = "btn-click-trigger"; +"Click" }
    ...
    div {
        Button { id = "btn-manual-trigger"; +"Manual (forced open)" }
        Tooltip(forId = "btn-manual-trigger", trigger = +manual, open = false) { p { +"Manual trigger; opened via open=true" } }
    }
}
...
fun Component.WithoutArrow() {
    h2 { +"Without Arrow" }
    div {
        Button { id = "btn-no-arrow-tip"; +"No Arrow" }
