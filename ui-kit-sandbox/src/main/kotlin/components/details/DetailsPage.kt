...
class DetailsPageController {
    ...
    @GetMapping
    fun render() = view(DetailsPage(), ::UiKitLayout)
}
...
fun Component.Grouped() {
    ...
    div {
        ...
        Details(summary = "Group A - Item 2", name = "groupA", open = true) { p { +"Opening this will close others in group A." } }
...
fun Component.Appearances() {
    ...
    Details(summary = "Appearance: filled + outlined", appearance = filled + outlined) {
...
fun Component.IconPositions() {
    ...
    Details(summary = "Icon at end (default)", iconPlacement = IconPlacement.end) { p { +"Icon is rendered at the end." } }
}
